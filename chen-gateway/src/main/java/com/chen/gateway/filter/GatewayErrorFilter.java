...
public class GatewayErrorFilter extends SendErrorFilter {
    ...
    public Object run() {
        try {
            ...
            mResponse = resolveExceptionMessage(message, serviceId, mResponse);

            HttpServletResponse response = ctx.getResponse();
            SystemUtil.makeResponse(
                    response, MediaType.APPLICATION_JSON_UTF8_VALUE,
                    HttpServletResponse.SC_INTERNAL_SERVER_ERROR, mResponse
            );
            log.error("Zull sendError：{}", mResponse.getMessage());
        } catch (Exception ex) {
    ...
    private MyResponse resolveExceptionMessage(String message, String serviceId, MyResponse febsResponse) {
